...
const getProductFromCategory = async (category: string): Promise<Product[]> => {
    //fake delay
  await new Promise((resolve) => setTimeout(resolve, 1500))

  return new Promise((resolve) => {
    resolve(product.filter((p: Product) => p.category === category))
  })
}

export { getProducts, getProduct, getCategories, getHighlightedProducts, getProductFromCategory }
